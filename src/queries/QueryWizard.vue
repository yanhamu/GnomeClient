...
export default {
  ...
  methods: {
    ...
    updateQuery: function() {
      var data = {};
      this.$http.put("queries/" + this.query.id, data).then();
    },
    createQueryObject: function(queryData) {
      ...
      queryData.accounts.forEach(a =>
        data.accounts.push(this.matchById(a, this.accounts))
      );
      queryData.includeExpressions.forEach(e =>
        data.includeExpressions.push(this.matchById(e, this.expressions))
      );
      queryData.excludeExpressions.forEach(e =>
        data.excludeExpressions.push(this.matchById(e, this.expressions))
      );
    ...
    create: function() {
      this.$http.post("queries", { name: "new-query" }).then(res => {
